...
def find_chull_edge():
    ...
    polygon_edges = set([(vs[i], vs[(i+1) % len(vs)]) for i in range(len(vs))])
    convex_hull_edges = set([(cs[i], cs[(i+1) % len(cs)]) for i in range(len(cs))])
    diff_edges = convex_hull_edges - polygon_edges
...
def redraw():
    global graph_editor_scene, vertices, edges, old_chain, blackStyle, blueStyle, redStyle, flip_edge
    graph_editor_scene.clear()
    vertices = PolygonE2(vertices).ccwOrientation().vertices
    
    cvx_hull = convex_hull(vertices)
    graph_editor_scene.add(cvx_hull, lightGrayStyle)
    ...
    if flip_edge[0] is not None:
        graph_editor_scene.add(SegmentE2(flip_edge[0], flip_edge[1]), redStyle)
    
    if old_chain:
        graph_editor_scene.addAll([SegmentE2(old_chain[i], old_chain[i + 1]) for i in range(len(old_chain) - 1)], blueStyle)
    # Draw vertices
...
def key_released(key):
    ...
    if key == "Option" or key == "Alt":
        ...
        if old_chain is not None:
            old_chain = [PointE2(p.x, p.y) for p in vertices[min(pi, pj):max(pi, pj) + 1]]
        redraw()
    elif key == "f":
        flip_edge = find_chull_edge()
        if flip_edge[0] is not None:
