...
blackStyle = makeStyle(stroke=(0, 0, 0), fill=(
    255, 255, 255), strokeWeight=4.0)
...
def find_chull_edge():
    ...
    for i in range(len(vertices)):
        edges.add((vertices[i], vertices[(i+1) % len(vertices)]))
    # print(edges)

    for j in range(hull.vertexCount):
        val = (hull.vertices[j], hull.vertices[(j+1) % hull.vertexCount])
        if val not in edges:
            return val
...
def redraw():
    ...
    hull = convex_hull(vertices)
    graph_editor_scene.add(hull, makeStyle(
        stroke=(200, 200, 200), fill=(240, 240, 240), strokeWeight=0.5))

    # Draw polygon
    graph_editor_scene.addAll([(SegmentE2(vertices[i], vertices[(
        i + 1) % len(vertices)]), blackStyle) for i in range(len(vertices))])

    if flip_edge:
        graph_editor_scene.add(flip_edge, redStyle)

    if old_chain:
        graph_editor_scene.addAll([(SegmentE2(
            old_chain[i], old_chain[i+1], blueStyle)for i in range(len(old_chain - 1)))])
...
def key_released(key):
    global option_key_down, mode, selected_vertex_idx, vertices, flip_edge, old_chain
    if key == "Option" or key == "Alt":
        option_key_down = False
    elif key == "v":
        mode = "vertex"
    ...
    # elif key == "c":
    #     ctr = (-1 / len(vertices)) *
    #     sum([VectorE2(p.x, p.y) for p in vertices], VectorE2(0, 0))
    #     vertices = [p + ctr for p in vertices]
    #     redraw()
    elif key == "f":
        p1, p2 = find_chull_edge()
        line = SegmentE2(p1, p2)
        flip_edge = line

        ind_1 = vertices.index(p1)
        ind_2 = vertices.index(p2)

        print(f"ind1 = {ind_1} ind2 = {ind_2}")

        old_chain = vertices[min(ind_1, ind_2):max(ind_1, ind_2)]
        if flip_edge:
            for val in range(min(ind_1, ind_2) + 1, max(ind_1, ind_2) - 1):
                vertices[val] = line.reflect(vertices[val])
