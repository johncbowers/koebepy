...
def rigidity_matrix_row(i, j):
    #Returns a row of the rigidity matrix for the edge between vertices i and j.
    global vertices
...
def rigidity_matrix():
    # Computes the full rigidity matrix for the graph
...
# Calculate null space of the TRANSPOSE
ns = null_space(R.T)

# this didn't work for me, I think I just didn't get syntax right but its hard to debug
# tensegrity_scene.addAll([SegmentE3(vertices[i], vertices[j], blueStyle if ns[idx] < 0 else redStyle if ns[idx] > 0 else blackStyle) for idx, (i, j) in enumerate(edges)])

    
stress_vector = ns[:, 0]
    
for idx, (i, j) in enumerate(edges):
    stress_value = stress_vector[idx] * -1
        
    # Color based on sign of stress value
    if stress_value > 1e-8:  # Positive stress - tension (blue)
        style = blueStyle
    elif stress_value < -1e-8:  # Negative stress - compression (red)  
        style = redStyle
    else:  # Zero stress (black)
        style = blackStyle
