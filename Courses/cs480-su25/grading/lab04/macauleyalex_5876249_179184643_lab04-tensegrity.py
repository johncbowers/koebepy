...
def rigidity_matrix():
    ...
    return np.array(
        [np.array(row).flatten() for row in edge_rows]
    )
...
for idx, (i, j) in enumerate(edges):
    if ns[idx] > 0:
        tensegrity_scene.add(SegmentE3(vertices[i], vertices[j]), blueStyle)
    elif ns[idx] < 0:
        tensegrity_scene.add(SegmentE3(vertices[i], vertices[j]), redStyle)
    else:
        tensegrity_scene.add(SegmentE3(vertices[i], vertices[j]), blackStyle)


tensegrity_scene.addAll((SegmentE3(vertices[i], vertices[j]), redStyle if ns[idx] < 0 else blueStyle) for idx, (i, j) in enumerate(edges))
