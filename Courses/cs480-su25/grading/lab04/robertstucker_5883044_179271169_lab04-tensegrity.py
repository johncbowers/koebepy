...
tensegrity_scene.addAll([(SegmentE3(vertices[i], vertices[j]), blueStyle) if n[idx] > 0 else (SegmentE3(vertices[i], vertices[j]), redStyle) if n[idx] < 0 else (SegmentE3(vertices[i], vertices[j]), blackStyle)  for idx, (i, j) in enumerate(edges)])
