...
def rigidity_matrix_row(i, j):
    ...
    row[i], row[j] = tuple(vertices[j]-vertices[i]), tuple(vertices[i]-vertices[j])
    return row

def rigidity_matrix():
    global vertices, edges
    matrix = [rigidity_matrix_row(i, j) for i, j in edges]
...
for idx, (i, j) in enumerate(edges):
    val = n[idx]
    style = blueStyle if val > 0 else redStyle if val < 0 else blackStyle
    seg = SegmentE3(vertices[i], vertices[j])
    segments.append((seg, style))

tensegrity_scene.addAll(segments)
