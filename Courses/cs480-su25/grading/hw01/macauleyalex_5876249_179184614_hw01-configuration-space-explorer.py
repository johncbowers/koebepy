    ...
    if len(configurations) == 2:
        ...
        for i in range(-250, 250, 10):
            for j in range(-250, 250, 10):
                theta1 = i * math.pi / 250
                theta2 = j * math.pi / 250
                arm1, arm2 = arm_segments(arm1_length, arm2_length, theta1, theta2)
        ...
        for (i, j) in valid_points:
            neighbors = []
            for di, dj in [(-10, 0), (10, 0), (0, -10), (0, 10)]:
                ni = i + di
                nj = j + dj
                # wrap around edges
                if ni < -250: ni = 240
        ...
        start = (round(configurations[0][0] * 250 / math.pi / 10) * 10, round(configurations[0][1] * 250 / math.pi / 10) * 10)
        goal = (round(configurations[1][0] * 250 / math.pi / 10) * 10, round(configurations[1][1] * 250 / math.pi / 10) * 10)
        queue = deque([start])
        ...
        for (i, j) in path:
            pt = PointE2(i, j)
            configuration_space_scene.add(pt, redStyle)
        ...
        for (i, j) in path:
            ...
            # viewer.pushAnimFrame()  Couldnt get it working, just glitched everytime and never ran the animation
            configuration_space_scene.add(PointE2(i, j), redStyle)

        viewer.show()
        theta1, theta2 = configurations[0]
        draw_mechanism(theta1, theta2, redStyle)
        theta1, theta2 = configurations[1]
