...
purpleStyle = makeStyle(stroke=(128, 0, 128),
                        fill=(128, 0, 128), strokeWeight=4.0)
...
def animation_helper(vertices, es, scene):
    current_v = vertices
    prev_v = None
    for _ in range(1200):
        # calculate current matric for current_v
        scene.clear()
        R = rigidity_matrix(current_v)

        # calculate vectors for the new rig matrix
        ns = null_space(R)
        if ns.shape[1] == 1 and selected_end_vertex_idx == -1:
            v = (ns.T)[0]
            if prev_v is not None and prev_v.dot(v) < 0:
                v = -v
            prev_v = v
            vectors = [VectorE2(float(v[val]), float(v[val + 1]))
                  for val in range(0, v.shape[0], 2)]

        # update the position using the equation
            for i in range(len(current_v)):
                current_v[i] = current_v[i] +  1 * vectors[i]

        # draw them to scene
        scene.addAll([(SegmentE2(current_v[e[0]], current_v[e[1]]),
                       purpleStyle if i == pin_edge_idx else blackStyle) for i, e in enumerate(es)])

        scene.addAll([(current_v[i], blackStyle if i != selected_vertex_idx and i !=
                       selected_end_vertex_idx else redStyle) for i in range(len(current_v))])
    # push animation frame scene.pushAnimationFrame()
        scene.pushAnimFrame()
...
def rigidity_matrix(verts):
    global edges, pin_edge_idx
    edge_rows = [rigidity_matrix_row(i, j, verts) for i, j in edges]
...
def redraw():
    ...
    if ns.shape[1] > 0:
        ...
        vs = [VectorE2(float(v[i]), float(v[i+1]))
    ...
    # Draw edges
    graph_editor_scene.addAll([(SegmentE2(vertices[e[0]], vertices[e[1]]),
                              purpleStyle if i == pin_edge_idx else blackStyle) for i, e in enumerate(edges)])
    ...
    if mouse_position is not None and selected_vertex_idx != -1:
        ...
        graph_editor_scene.add(SegmentE2(
            vertices[selected_vertex_idx], mouse_position if selected_end_vertex_idx == -1 else vertices[selected_end_vertex_idx]), redStyle)

    # Draw vertices
    graph_editor_scene.addAll([(vertices[i], blackStyle if i != selected_vertex_idx and i !=
                              selected_end_vertex_idx else redStyle) for i in range(len(vertices))])

    # don't forget to call your helper
    animation_helper(vertices, edges, motion_scene)
...
def mouse_dragged(event):
    ...
    if mode == "edge" and selected_vertex_idx != -1:
        ...
        mouse_position = PointE2(event["x"], event["y"])
        selected_end_vertex_idx = nearest_vertex_idx_of(
    ...
    if mode == "edge":
        if selected_vertex_idx != -1 and selected_end_vertex_idx != -1 and selected_vertex_idx != selected_end_vertex_idx:
            ...
            if [selected_vertex_idx, selected_end_vertex_idx] not in edges and [selected_end_vertex_idx, selected_vertex_idx] not in edges:
                ...
                print(
                    f"Added edge: {selected_vertex_idx} -> {selected_end_vertex_idx}")
