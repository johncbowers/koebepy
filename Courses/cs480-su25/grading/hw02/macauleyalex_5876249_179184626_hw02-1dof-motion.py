...
def rigidity_matrix(edges, vertices):
    global pin_edge_idx
...
def draw_scene(vertices, edges, scene):
    # Draw edges
    scene.addAll([(SegmentE2(vertices[e[0]], vertices[e[1]]), purpleStyle if i == pin_edge_idx 
                   else blackStyle) for i, e in enumerate(edges)])
    # Draw vertices
    scene.addAll([(vertices[i], blackStyle if i != selected_vertex_idx and i != selected_end_vertex_idx 
                   else redStyle) for i in range(len(vertices))])
...
def redraw():
    ...
    R = rigidity_matrix(edges, vertices)
    print_dof(R)
    ns = null_space(R)
    ...
    draw_scene(vertices, edges, graph_editor_scene)
    ...
    if ns.shape[1]  == 1:
        v_curr = vertices
        r_curr = rigidity_matrix(edges, vertices)
        draw_scene(v_curr, edges, motion_scene)
        for frame in range(800):
            ns_curr = null_space(r_curr)
            if ns_curr.shape[1] != 1:
                break
            v = ns_curr[:, 0]
            if prev_v is not None and prev_v.dot(v) < 0:
                v = -v
            prev_v = v
            vs_prime = [VectorE2(float(v[i]), float(v[i+1])) for i in range(0, v.shape[0], 2)]
            v_new = [v_curr[i] + 0.5 * vs_prime[i] for i in range(len(v_curr))]

            draw_scene(v_new, edges, motion_scene)
            motion_scene.pushAnimFrame()

            v_curr = v_new
            r_curr = rigidity_matrix(edges, v_curr)
