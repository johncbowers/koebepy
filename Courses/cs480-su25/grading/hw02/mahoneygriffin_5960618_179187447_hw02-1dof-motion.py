...
def rigidity_matrix(edges, pin_edge_idx, vertices):
...
def motion(edges, vertices, scene):
    global step_amount
    prev_v = None
    for _ in range(motion_length):
        motion_R = rigidity_matrix(edges, pin_edge_idx, vertices)
        ns = null_space(motion_R)
        v = (ns.T)[0]
        if prev_v is not None and prev_v.dot(v) < 0:
            v = -v
        prev_v = v
        vs = [VectorE2(float(v[i]), float(v[i + 1])) for i in range(0, v.shape[0], 2)]
        vertices = [vertices[i] + step_amount * vs[i] for i in range(len(vertices))]
        draw(edges, vertices, scene)
        scene.pushAnimFrame()
...
prev_v = None
def redraw():
    global graph_editor_scene, vertices, edges, prev_v, motion_scene
    graph_editor_scene.clear()
    motion_scene.clearAnimFrames()

    R = rigidity_matrix(edges, pin_edge_idx, vertices)
    print_dof(R)
    ns = null_space(R)
    ...
    if ns.shape[1] == 1 and selected_end_vertex_idx == -1:
        motion([x[:] for x in edges], [PointE2(v.x, v.y) for v in vertices], motion_scene)
    ...
    draw(edges, vertices, graph_editor_scene)

def draw(edges, vertices, scene):
    scene.addAll([(SegmentE2(vertices[e[0]], vertices[e[1]]), purpleStyle if i == pin_edge_idx else blackStyle) for i, e in enumerate(edges)])
    scene.addAll([(vertices[i], blackStyle if i != selected_vertex_idx and i != selected_end_vertex_idx else redStyle) for i in range(len(vertices))])
#
# UI Handling
#
