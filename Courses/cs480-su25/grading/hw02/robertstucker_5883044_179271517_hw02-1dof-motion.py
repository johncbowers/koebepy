# Should work for most cases, Had some weird stuff happen when trying to switch modes, adding vertices or edges, got this error message in my terminal.
# Image              PC                Routine            Line        Source
# libifcoremd.dll    00007FFA953BDF54  Unknown               Unknown  Unknown
# KERNELBASE.dll     00007FFAFCAFB0CD  Unknown               Unknown  Unknown
# KERNEL32.DLL       00007FFAFE05E8D7  Unknown               Unknown  Unknown
# ntdll.dll          00007FFAFF85C34C  Unknown               Unknown  Unknown
# Would still work, so I think its fine.
...
# frame of the animation. I would experiment with different numbers of frames, but 200
# might be good. You also may want to add a global variable delta that you use to scale
# the null_space vector by to change how much you step along the tangent to your motion
# each time. 
# 
# Hints: 
#
...
def rigidity_matrix(vertices, edges, pin_edge_idx):
    edge_rows = [rigidity_matrix_row(i, j, vertices) for i, j in edges]
    i, j = edges[pin_edge_idx]
    R = edge_rows + pinning_rows(i, vertices)+pinning_rows(j, vertices)
    return np.array(
...
def redraw():
    global graph_editor_scene, motion_scene, vertices, edges, prev_v
    ...
    if ns.shape[1] == 1 and selected_vertex_idx == -1:
        motion_verts = vertices
        motion_prev_v = None
        motion_v = v
        motion_R = R
        draw_graph_to_scene(motion_verts, edges, motion_scene)
        for i in range(1400):
            motion_scene.pushAnimFrame()
            motion_vs = [VectorE2(float(motion_v[i]), float(motion_v[i+1])) for i in range(0, motion_v.shape[0], 2)]
            motion_verts = [p + motion_vs[i] for i, p in enumerate(motion_verts)]
            draw_graph_to_scene(motion_verts, edges, motion_scene)
            motion_R = rigidity_matrix(motion_verts, edges, pin_edge_idx)
            motion_ns = null_space(motion_R)
            if motion_ns.shape[1] == 1:
                motion_v = (motion_ns.T)[0]
                if motion_prev_v is not None and motion_prev_v.dot(motion_v) < 0:
                    motion_v = -motion_v
                motion_prev_v = motion_v
        # for i in range(1400):
        #     motion_vs = [VectorE2(float(motion_v[i]), float(motion_v[i+1])) for i in range(0, motion_v.shape[0], 2)]
        #     motion_verts = [p + motion_vs[i] for i, p in enumerate(motion_verts)]
        #     draw_graph_to_scene(motion_verts, edges, motion_scene)
    ...
    draw_graph_to_scene(vertices, edges, graph_editor_scene)

def draw_graph_to_scene(vertices, edges, scene):
    scene.addAll([(SegmentE2(vertices[e[0]], vertices[e[1]]), purpleStyle if idx == pin_edge_idx else blackStyle) 
                  for idx, e in enumerate(edges)])
    scene.addAll([(vertices[idx], blackStyle if idx != selected_vertex_idx and idx != selected_end_vertex_idx else redStyle) 
