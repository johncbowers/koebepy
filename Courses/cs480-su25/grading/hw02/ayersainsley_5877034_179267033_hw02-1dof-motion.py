...
#draw the graph on the other scene
#make a new frame of the animation
# next vertices = vertices[i] + delta * vs[i]

#current vertices = vertices
#for frameidk in range():
    #in here i would need to calculate it each time for when I draw it again
    #calc rigidty matrix for current vertices
    #calculate vs for current regidity matrix
    #update position of the vertices based on vs
    #draw them

#take a step
#add a drawing to motion_scene
#pushAnimFrame
...
#this is my function for doing all the stuff???
def graph_drawing_func(): 
    for frame in range(1200):

        #calc rigidity matrix
        #calc null space
        new_vertices = []
        for i in range(len(vertices)):
            new_vertices.push(
                vertices[i] + vs[i] #i'll need to figure out vs
            )
    #draw edges + new vertices of graph
    #push anim
