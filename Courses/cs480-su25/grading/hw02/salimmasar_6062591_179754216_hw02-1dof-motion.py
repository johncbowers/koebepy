...
def draw_framework_for_anim(scene, p_vertices, p_edges, p_pin_edge_idx):
    # Draw edges
    scene.addAll([(SegmentE2(p_vertices[e[0]], p_vertices[e[1]]), purpleStyle if i == p_pin_edge_idx else blackStyle) for i, e in enumerate(p_edges)])
    # Draw vertices
    scene.addAll([(p, blackStyle) for p in p_vertices])
...
def redraw():
    ...
    if ns.shape[1] == 1:
        ...
        motion_vertices = [p for p in vertices]
        v_current = prev_v
        step_size = 0.5
        num_frames = 200

        original_vertices_backup = vertices

        for _ in range(num_frames):
            for i in range(len(motion_vertices)):
                vel = VectorE2(float(v_current[2*i]), float(v_current[2*i+1]))
                motion_vertices[i] = motion_vertices[i] + step_size * vel

            draw_framework_for_anim(motion_scene, motion_vertices, edges, pin_edge_idx)
            motion_scene.pushAnimFrame()


            vertices = motion_vertices
            R_anim = rigidity_matrix()
            

            vertices = original_vertices_backup
            
            ns_anim = null_space(R_anim)
            if ns_anim.shape[1] != 1:
                break
            
            v_next = (ns_anim.T)[0]
            if v_current.dot(v_next) < 0:
                v_next = -v_next
