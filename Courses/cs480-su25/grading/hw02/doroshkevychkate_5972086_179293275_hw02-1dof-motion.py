...
#p - vertices, G - graph
def rigidity_matrix_param(edges, vertices):
    global pin_edge_idx
    edge_rows = [rigidity_mtx_row_custom(i, j, vertices) for i, j in edges]
    i, j = edges[pin_edge_idx]
    R = edge_rows + pinning_rows_custom(i, vertices)+pinning_rows_custom(j, vertices)
    return np.array(
        [np.array(row).flatten() for row in R]
    )

def rigidity_mtx_row_custom(i, j, vert):
    row = [(0, 0) for _ in vert]
    row[i] = tuple(vert[j]-vert[i])
    row[j] = tuple(vert[i]-vert[j])
    return row
...
def rigidity_matrix_row(i, j):
    ...
    return row
def pinning_rows_custom(i, vert):
    
    row1 = [(0, 0) for _ in vert]
    row2 = [(0, 0) for _ in vert]
    row1[i] = (1, 0)
    row2[i] = (0, 1)
    return [row1, row2]
...
def redraw():
    ...
    prev_v_m = None
    if ns.shape[1] == 1 and selected_vertex_idx == -1:    #1 degree of freedom?
        current_vertices = vertices
        for frm in range(1200):
            R = rigidity_matrix_param(edges, current_vertices)
            ns = null_space(R)

            v_m = (ns.T)[0]
            
            if prev_v_m is not None and prev_v_m.dot(v_m) < 0:
                v_m = -v_m
            prev_v_m = v_m
            
            vs_m = [VectorE2(float(v_m[i]), float(v_m[i+1])) for i in range(0, v_m.shape[0], 2)]
            current_vertices = [current_vertices[i] + vs_m[i] for i in range(len(current_vertices))]

            motion_scene.addAll([(SegmentE2(current_vertices[e[0]], current_vertices[e[1]]), purpleStyle if i == pin_edge_idx else blackStyle) for i, e in enumerate(edges)])
    
    

    # Draw vertices
            motion_scene.addAll([(current_vertices[i], blackStyle) for i in range(len(vertices))])
