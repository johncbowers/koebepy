...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for i in range(-250, 250, 10):
        for j in range(-250, 250, 10):
            position = PointE2(i, j)
            temp_ship = ship_polygon(position, ship_rotation)

            for k in range(len(obstacles)):
                if(temp_ship.intersects(obstacles[k])):
                    crash = bool(True)
            if(crash):
                configuration_space_1.add(position, blackStyle)
            crash = bool(False)
    ...
    for x in range(-250, 250, 10):
        for t in range(0, 360, 7):
            position = PointE2(x, ship_position.y)
            temp_ship = ship_polygon(position, radians(t))
            for i in range(len(obstacles)):
                if(temp_ship.intersects(obstacles[i])):
                    crash = bool(True)
                
            if(crash):
                configuration_space_2.add(PointE2(x, 500 * (t/360) - 250), blackStyle)
            crash = bool(False)
    # TODO
    ...
    for x in range(-250, 250, 25):
        for y in range(-250, 250, 25):
            for t in range(0, 360, 18):
                pos2d = PointE2(x, y)
                temp_ship = ship_polygon(pos2d, radians(t))
                ...
                if(crash):
                    configuration_space_3.add(PointE3(x / 250 , y / 250 , t / 180 - 1.0), greenStyle)
                crash = bool(False)
