...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for x in range(-250, 250, 10):
        for y in range (-250, 250, 10):
            intersect_counter1 = 0
            ship1 = ship_polygon(PointE2(x, y), ship_rotation)
            for z in obstacles:
                if ship1.intersects(z):
                    intersect_counter1 = 1
            if (intersect_counter1 == 1):
                configuration_space_1.add(PointE2(x, y), blackStyle)
    ...
    for x in range(-250, 250, 10):
        for theta in range(0, 360, 7):
            intersect_counter2 = 0
            rad = radians(theta)
            ship2 = ship_polygon(PointE2(x, ship_position.y), rad)
            for z in obstacles:
                if ship2.intersects(z):
                    intersect_counter2 = 1
            if (intersect_counter2 == 1):
                configuration_space_2.add(PointE2(x, (500 * (theta / 360) - 250)), blackStyle)
    ...
    for x in range(-250, 250, 25):
        for y in range(-250, 250, 25):
            for theta in range(0, 360, 18):
                intersect_counter3 = 0
                rad = radians(theta)
                ship3 = ship_polygon(PointE2(x, y), rad)
                for z in obstacles:
                    if ship3.intersects(z):
                        intersect_counter3 = 1
                if (intersect_counter3 == 1):
                    configuration_space_3.add(PointE3((x/250), (y/250), (theta/180 - 1.0)), greenStyle)
