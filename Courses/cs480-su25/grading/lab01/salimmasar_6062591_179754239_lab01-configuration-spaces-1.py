...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for x in range(-250,250,10):
        for y in range(-250,250,10):
            temp_pos = PointE2(x,y)
            temp_poly = ship_polygon(temp_pos, ship_rotation)

            if not any(temp_poly.intersects(ob) for ob in obstacles):
    ...
    for x in range(-250, 250, 10):
        for theta_deg in range(0, 360, 7):
            ...
            temp_pos = PointE2(x, ship_position.y)
            temp_poly = ship_polygon(temp_pos, theta_rad)

            if not any(temp_poly.intersects(ob) for ob in obstacles):
                y_coord = 500 * (theta_deg / 360.0) - 250
                configuration_space_2.add(PointE2(x, y_coord), blackStyle)
        ...
        for y in range(-250, 250, 25):
            for theta_deg in range(0, 360, 18):
                ...
                if not any(temp_poly.intersects(ob) for ob in obstacles):
                    p3_x = x / 250.0
                    p3_y = y / 250.0
                    p3_z = theta_deg / 180.0 - 1.0
                    configuration_space_3.add(PointE3(p3_x, p3_y, p3_z), greenStyle)
