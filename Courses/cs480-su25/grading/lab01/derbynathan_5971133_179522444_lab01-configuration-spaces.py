...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for x in range (-250, 251, 10):
        for y in range (-250, 251, 10):
            point = PointE2(x,y)
            ship_poly_at_point = ship_polygon(point, ship_rotation)
            if any(ob.intersects(ship_poly_at_point) for ob in obstacles):
    ...
    for x in range (-250, 251, 10):
        for theta in range (0, 361, 7):
            point = PointE2(x, ship_position.y)
            ship_poly_at_point = ship_polygon(point, radians(theta))
            if any(ob.intersects(ship_poly_at_point) for ob in obstacles):
    ...
    for x in range (-250, 251, 25):
        for y in range (-250, 251, 25):
            for theta in range (0, 361, 18):
                point = PointE3 (x, y, radians(theta))
                ship_poly_at_point = ship_polygon(PointE2(x, y), radians(theta))
                if any(ob.intersects(ship_poly_at_point) for ob in obstacles):
