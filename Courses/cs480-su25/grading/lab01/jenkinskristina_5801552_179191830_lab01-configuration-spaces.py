...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for x in range(-250, 250, 10):
        for y in range(-250, 250, 10):
            spot = PointE2(x, y)
            new_whip = ship_polygon(spot, ship_rotation)

            if all(not new_whip.intersects(ob) for ob in obstacles):
                    configuration_space_1.add(spot, blackStyle)
    ...
    for x in range(-250, 250, 10):
        for theta_deg in range(0, 360, 7):
            ...
            spot2 = PointE2(x, ship_position.y)
            whip2 = ship_polygon(spot2, theta_rad)

            if all(not whip2.intersects(ob) for ob in obstacles):
                viewer_y = 500 * (theta_deg / 360) - 250
                configuration_space_2.add(PointE2(x, viewer_y), blackStyle)
    ...
    for x in range(-250, 251, 25):
        for y in range(-250, 251, 25):
            for theta_deg in range(0, 361, 18):
                theta_rad = radians(theta_deg)
                spot3 = PointE2(x, y)
                whip3 = ship_polygon(spot3, theta_rad)

                if all(not whip3.intersects(ob) for ob in obstacles):
                    viewer_x = x / 250
                    viewer_y = y / 250
                    viewer_z = theta_deg / 180 - 1.0
                    configuration_space_3.add(PointE3(viewer_x, viewer_y, viewer_z), greenStyle)
