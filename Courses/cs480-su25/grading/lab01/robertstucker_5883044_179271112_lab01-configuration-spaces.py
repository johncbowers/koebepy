...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    def helper(thing):
                for o in obstacles:
                    if thing.intersects(o):
                        return False
                return True
    for x in range(-250, 250, 10):
        for y in range(-250, 250, 10):
            tmp_ship = ship_polygon(PointE2(x, y), ship_rotation)
            if helper(tmp_ship):
    ...
    curr_y = ship_position.y
    for x in range(-250, 250, 10):
        for deg in range(0, 360, 7):
            tmp_ship = ship_polygon(PointE2(x, curr_y), radians(deg))
            if helper(tmp_ship):
                configuration_space_2.add(PointE2(x, 500 * (deg / 360) - 250), blackStyle)
    ...
    for x in range(-250, 250, 25):
        for y in range(-250, 250, 25):
            for deg in range(0, 360, 18):
                tmp_ship = ship_polygon(PointE2(x, y), radians(deg))
                if helper(tmp_ship):
                    configuration_space_3.add(PointE3(x / 250, y / 250, deg / 180 - 1.0), greenStyle)
