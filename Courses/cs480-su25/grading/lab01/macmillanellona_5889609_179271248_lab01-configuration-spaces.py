...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for x in range (-250, 250, 10):
        for y in range (-250, 250, 10):
            new_ship = ship_polygon (PointE2(x,y), ship_rotation)
            for ob in obstacles:
                if ob.intersects(new_ship):
    ...
    for x in range (-250, 250, 10):
        for theta in range (0, 360, 7):
            new_ship = ship_polygon (PointE2 (x, ship_position.y), radians(theta))
            for ob in obstacles:
                if ob.intersects(new_ship):
    ...
    for x in range (-250, 250, 25):
        for y in range (-250, 250, 25):
            for theta in range (0, 360, 18):
                new_ship = ship_polygon (PointE2 (x, y), radians (theta))
                for ob in obstacles:
                    if ob.intersects(new_ship):
