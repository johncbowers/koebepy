...
def generate_asteroid_scene_geometry(asteroids_scene, configuration_space_1, configuration_space_2, configuration_space_3):
    ...
    for x in range(-250, 250, 10):
        for y in range(-250, 250, 10):
            sample_position = PointE2(x, y)
            sample_poly = ship_polygon(sample_position, ship_rotation)
            for ob in obstacles:
                if sample_poly.intersects(ob):
                    configuration_space_1.add(sample_position, blackStyle)
    ...
    for x in range(-250, 250, 10): #grid of x values between -250 and 250
        for t in range (0, 360, 7): #grid of rotation values between 0 and 360
            sample_position = PointE2(x, ship_position.y) #using ships y and the x coords
            sample_poly = ship_polygon(sample_position, radians(t)) #ship polygon at (x, y)
            for ob in obstacles:
                if sample_poly.intersects(ob):
                    obstacle_point = PointE2(x, 500 * (t / 360) - 250)
                    configuration_space_2.add(obstacle_point, blackStyle)
    ...
    for x in range(-250, 250, 25):
        for y in range(-250, 250, 25):
            for z in range(0, 360, 18):
                #ship's current position + x,y,z to create PointE3 obj
                sample_position = PointE3(x, y, z / 180 - 1.0)
                sample_ship = PointE2(sample_position.x, sample_position.y)
                sample_poly = ship_polygon(sample_ship, z / 180 - 1.0)
                #if ship polygon at (x, y, z) does not intersect obstacles
                for ob in obstacles:
                    if sample_poly.intersects(ob):
                        obstacle_point = PointE3(x / 250, y / 250, z / 180 - 1.0)
                        configuration_space_3.add(obstacle_point, greenStyle)
