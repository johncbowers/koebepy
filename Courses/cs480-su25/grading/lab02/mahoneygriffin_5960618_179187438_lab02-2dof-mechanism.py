...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    arm1_end_x = arm1_length * math.cos(theta1)
    arm1_end_y = arm1_length * math.sin(theta1)
    arm1_end = PointE2(arm1_end_x, arm1_end_y)

    arm2_end_x = arm1_end_x + arm2_length * math.cos(theta1 + theta2)
    arm2_end_y = arm1_end_y + arm2_length * math.sin(theta1 + theta2)
    arm2_end = PointE2(arm2_end_x, arm2_end_y)

    arm1_segment = SegmentE2(PointE2(0, 0), arm1_end)
    arm2_segment = SegmentE2(arm1_end, arm2_end)

    return arm1_segment, arm2_segment
...
for my_theta1 in range(-250, 250, 10):
    for my_theta2 in range(-250, 250, 10):
        temp_theta1 = radians(my_theta1/(250/180))
        temp_theta2 = radians(my_theta2/(250/180))
        segment1, segment2 = arm_segments(arm1_length, arm2_length, temp_theta1, temp_theta2)
        if (segment1.intersects(obstacle1) or segment1.intersects(obstacle2)) or (segment2.intersects(obstacle1) or segment2.intersects(obstacle2)):
            continue
        configuration_space_scene.addToBackground(PointE2(my_theta1, my_theta2), grayFillStyle)
