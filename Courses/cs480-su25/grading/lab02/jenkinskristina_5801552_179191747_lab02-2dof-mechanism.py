...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    arm1 = SegmentE2(PointE2(0, 0), PointE2(
        arm1_length * math.cos(theta1), arm1_length * math.sin(theta1)))
    

    x1 = PointE2(arm1_length * math.cos(theta1), arm1_length * math.sin(theta1))
    y1 = PointE2(arm1_length * math.cos(
        theta1) + arm2_length * math.cos(theta1 + theta2), arm1_length * math.sin(theta1) + arm2_length * math.sin(theta1 + theta2))
    arm2 = SegmentE2(x1, y1)
    # arm1_length * math.cos(theta1) + arm2_length * math.cos(theta1 + theta2), arm1_length * math.sin(theta1) + arm2_length * math.sin(theta1 + theta2)
...
def draw_configuration_space():
    ...
    configuration_space_scene.add(
        theta_pt, redStyle if selected else blueStyle)
...
for t1 in range(-250, 250, 10):
    for t2 in range(-250, 250, 10):
        arms1, arms2 = arm_segments(arm1_length, arm2_length,
                            (t1 * math.pi / 250), (t2 * math.pi / 250))
        if not (arms1.intersects(obstacle1) or arms1.intersects(obstacle2) or arms2.intersects(obstacle1) or arms2.intersects(obstacle2)):
            configuration_space_scene.addToBackground(PointE2(t1, t2), grayFillStyle)

    

configuration_space_scene.addToBackground(
    SegmentE2(PointE2(-250, 0), PointE2(250, 0)), blackStyle)
configuration_space_scene.addToBackground(
    SegmentE2(PointE2(0, -250), PointE2(0, 250)), blackStyle)
