...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    arm1 = SegmentE2(PointE2(0, 0), PointE2(arm1_length * (math.cos(theta1)), arm1_length * (math.sin(theta1))))
    arm2 = SegmentE2(PointE2(arm1_length * (math.cos(theta1)), arm1_length * (math.sin(theta1))),
         PointE2((arm1_length * (math.cos(theta1))) + arm2_length * (math.cos(theta2)), (arm1_length * (math.sin(theta1))) + arm2_length * (math.sin(theta2))))
    #arm1 = arm1_length(cos(theta1))
    #for arm2 = arm1_length(cos(theta1)) + arm2_length cos(theta2)
...
#theta1 values between -250 and 250 each spaced 10 units apart
for t1 in range(-250, 250, 10):  
    #theta2 values between -250 and 250 each spaced 10 units apart
    for t2 in range(-250, 250, 10):
        #for each coord use current arm lengths and theta1 and theta2 values to create two arm segments (armsegments function)
        rad_t1 = (t1 / 250) * math.pi
        rad_t2 = (t2 / 250) * math.pi
        a1, a2 = arm_segments(arm1_length, arm2_length, rad_t1, rad_t2)
        #if the arm segments do not intersect any obstacles
        if a1.intersects(obstacle1) or a1.intersects(obstacle2) or a2.intersects(obstacle1) or a2.intersects(obstacle2):
            point = PointE2(t1, t2)
            configuration_space_scene.addToBackground(point, grayFillStyle)
