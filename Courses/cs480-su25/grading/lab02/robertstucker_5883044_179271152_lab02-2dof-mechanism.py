...
obstacle3 = CircleE2(PointE2(-50, 150), 20)
...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    arm1x, arm1y = arm1_length * (math.cos(theta1)), arm1_length * (math.sin(theta1))
    arm2x, arm2y = (arm2_length * (math.cos(theta1+theta2)))+arm1x, (arm2_length * (math.sin(theta1+theta2)))+arm1y

    arm1 = SegmentE2(PointE2(0, 0), PointE2(arm1x, arm1y))
    arm2 = SegmentE2(PointE2(arm1x, arm1y), PointE2(arm2x, arm2y))

    return arm1, arm2

def draw_mechanism():
    global mech_scene, theta1, theta2, obstacle1, obstacle2, obstacle3, arm1_length, arm2_length
    ...
    mech_scene.add(obstacle3, blackStyle)
...
def mouse_moved(event):
    global mouse_down, theta1, theta2, selected, obstacles
...
for t1 in range(-250, 250, 10):
    t1_rad = (t1/250) * math.pi
    for t2 in range(-250, 250, 10):
        t2_rad = (t2/250) * math.pi
        test1, test2 = arm_segments(arm1_length, arm2_length, t1_rad, t2_rad)
        def helper(test1, test2):
            for o in obstacles:
                if test1.intersects(o) or test2.intersects(o):
                    return False
            return True
        
        if helper(test1, test2):
