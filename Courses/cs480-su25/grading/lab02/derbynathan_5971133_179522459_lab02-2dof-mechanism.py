...
for theta1_val in range(-250, 251, 10):
    for theta2_val in range(-250, 251, 10):
        # Convert from display coordinates to radians
        theta1_rad = theta1_val * math.pi / 250
        theta2_rad = theta2_val * math.pi / 250
        
        # Create arm segments for this configuration
        arm1, arm2 = arm_segments(arm1_length, arm2_length, theta1_rad, theta2_rad)
        
        # Check if either arm intersects with either obstacle
        if not (arm1.intersects(obstacle1) or arm1.intersects(obstacle2) or
                arm2.intersects(obstacle1) or arm2.intersects(obstacle2)):
            # Add point to configuration space if no collision
            configuration_space_scene.addToBackground(PointE2(theta1_val, theta2_val), grayFillStyle)
