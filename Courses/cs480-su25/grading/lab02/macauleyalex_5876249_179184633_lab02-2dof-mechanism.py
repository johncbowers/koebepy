...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    arm2 = SegmentE2(PointE2(arm1_length * math.cos(theta1), arm1_length * math.sin(theta1)), PointE2((arm1_length * math.cos(theta1)) + arm2_length * math.cos(theta1 + theta2),(arm1_length*math.sin(theta1)) + arm2_length * math.sin(theta1 + theta2)))
...
for temp_theta1 in range(-250, 250, 10):
    for temp_theta2 in range(-250, 250, 10):
        flag = 0
        theta1_prime = math.pi * (temp_theta1/250)
        theta2_prime = math.pi * (temp_theta2/250)
        test_segment1 = arm_segments(arm1_length, arm2_length, theta1_prime, theta2_prime)
        for segment in test_segment1:
            if (segment.intersects(obstacle1)):
                flag = 1
            if (segment.intersects(obstacle2)):
                flag = 1
        if flag == 0:
            configuration_space_scene.addToBackground(PointE2(temp_theta1, temp_theta2), grayFillStyle)
