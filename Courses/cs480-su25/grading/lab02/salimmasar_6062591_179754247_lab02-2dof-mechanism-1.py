...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    x1 = arm1_length * cos(theta1)
    y1 = arm1_length * sin(theta1)
    joint = PointE2(x1,y1)

    x2 = x1 + arm2_length * cos(theta1 + theta2)
    y2 = y1 + arm2_length * sin(theta1 + theta2)
    end = PointE2(x2,y2)


    # These are dummy arms to fix: 
    arm1 = SegmentE2(PointE2(0, 0), joint)
    arm2 = SegmentE2(joint, end)
...
for t1_pixel in range(-251,250,10): 
    for t2_pixel in range(-251,250,10): 
        t1 = t1_pixel * pi / 250
        t2 = t2_pixel * pi / 250

        arm1, arm2 = arm_segments(arm1_length, arm2_length, t1, t2)

        hit = arm1.intersects(obstacle1) or arm1.intersects(obstacle2) or arm2.intersects(obstacle1) or arm2.intersects(obstacle2)

        if not hit: 
            point = PointE2(float(t1_pixel), float(t2_pixel))
