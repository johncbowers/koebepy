...
def arm_segments(arm1_length, arm2_length, theta1, theta2):
    ...
    x = arm1_length * cos (theta1)
    y =  arm1_length * sin (theta1)
    # These are dummy arms to fix:
    arm1 = SegmentE2(PointE2(0, 0), PointE2(x, y))
    arm2 = SegmentE2(PointE2(x,y),
                     PointE2(x + (arm2_length * cos (theta1 + theta2)), y + (arm2_length * sin (theta2 + theta1))))
...
for t1 in range (-250, 250, 10):
    for t2 in range (-250, 250, 10):
        arm1, arm2 = arm_segments (arm1_length, arm2_length, (t1/250) * math.pi, (t2/250) * math.pi)
        if not arm1.intersects(obstacle1) and not arm1.intersects(obstacle2) and not arm2.intersects(obstacle1) and not arm2.intersects(obstacle2):
