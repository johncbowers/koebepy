...
def rigidity_matrix_row(i, j):
    ...
    row[i], row[j] = tuple(vertices[j]-vertices[j]), tuple(vertices[i]-vertices[j])
    return row
...
def pinning_row(i):
    global vertices
    rowx = [(0, 0) for _ in vertices]
    rowy = [(0, 0) for _ in vertices]
    rowx[i], rowy[i] = (1, 0), (0, 1)
    return [rowx, rowy]
...
def rigidity_matrix():
    global vertices, edges, pin_edge_idx
    rigidity_rows = [rigidity_matrix_row(i, j) for i, j in edges]
    i, j = edges[pin_edge_idx]
    R = rigidity_rows + pinning_row(i) + pinning_row(j)
...
def print_dof(R):
    ns = null_space(R)
    print(f"This mechanisms has {ns.shape[1]} degree of freedom")
#
...
def redraw():
    ...
    # Draw edges
    graph_editor_scene.addAll([
        (SegmentE2(vertices[e[0]], vertices[e[1]]), purpleStyle if i == pin_edge_idx else blackStyle) for i, e in enumerate(edges)])
