...
def pinning_row(i):
    ...
    row[i] = (1, 0)
    row2[i] = (0, 1)
    # return np.array([np.array(row).flatten(), np.array(row2).flatten])
    return [row, row2]
...
def rigidity_matrix():
    ...
    matrix = [rigidity_matrix_row(x, y) for x, y in edges]
    i, j = edges[pin_edge_idx]
    R = matrix + pinning_row(i) + pinning_row(j)
    return np.array([np.array(row).flatten() for row in R])
...
def redraw():
    global graph_editor_scene, vertices, edges, prev
    ...
    if ns.shape[1] > 0:
        v = (ns.T)[0]
        if prev is not None and prev.dot(v) < 0:
            v = -v
        prev = v

        vs = [VectorE2(float(v[val]), float(v[val + 1]))
              for val in range(0, v.shape[0], 2)]

        graph_editor_scene.addAll((SegmentE2(
            vertices[i], vertices[i] + 100 * vs[i]), redStyle) for i in range(len(vs)))
...
def mouse_pressed(event):
    global graph_editor_scene, vertices, edges, mode, option_key_down, selected_vertex_idx, nearest_vertex, selected_vertex_idx, mouse_position, prev
