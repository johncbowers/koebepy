...
# 3. Write a function that computes the rigidity matrix for the entire graph
...
def rigidity_matrix():
    ...
    matrix = edge_rows + pinning_rows(i) + pinning_rows(j)
    return np.array([np.array(row).flatten() for row in matrix])
...
def redraw():
    ...
    if ns.shape[1] > 0:
        ...
        vs = [VectorE2(float(v[i]), float(v[i+1])) for i in range(0, ns.shape[0], 2)]
        graph_editor_scene.addAll([
            (SegmentE2(vertices[i], vertices[i] + 200 * vs[i]), redStyle) for i in range(len(vs))
        ])

    # Draw edges
    graph_editor_scene.addAll([
        (SegmentE2(vertices[e[0]], vertices[e[1]]), purpleStyle if i == pin_edge_idx else blackStyle)
    ...
    if mouse_position is not None and selected_vertex_idx != -1:
        ...
        target = mouse_position if selected_end_vertex_idx == -1 else vertices[selected_end_vertex_idx]
        graph_editor_scene.add(SegmentE2(vertices[selected_vertex_idx], target), redStyle)
...
def mouse_pressed(event):
    global graph_editor_scene, vertices, edges, mode, option_key_down, selected_vertex_idx, mouse_position
    if mode == "vertex":
