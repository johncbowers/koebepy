...
# [(pattern for creating item) for whatever in whatever] - list comprehension :D
...
def rigidity_matrix_row (i, j):
    ...
    row = [(0,0) for _ in vertices] # zeros for each vertex
    row [i] = tuple (vertices[j] - vertices[i])
    row [j] = tuple (vertices[i] - vertices[j])

    # everything is a tuple, need to seperate for column access with np
    return row
...
def rigidity_matrix ():
    ...
    # call our previous function to compute the rigidity matrix for the graph
    edge_rows = [rigidity_matrix_row (i,j) for i,j in edges]
    i, j = edges[pin_edge_idx]
    R = edge_rows + pinning_rows(i) + pinning_rows(j)

    # flatten all the rows and now we have our known values
    return np.array(
...
def redraw():
    ...
    if ns.shape[1] > 0:
        # extract at least one solution
        v = (ns.T)[0]

        # to prevent flashing of the red line between moving around
