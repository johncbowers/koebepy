...
def pinning_rows(i):
    global vertices
    row_1 = [(0, 0) for _ in vertices]
    row_1[i] = (1, 0)
    row_2 = [(0, 0) for _ in vertices]
    row_2[i] = (0, 1)
    return [row_1 , row_2]
...
def redraw():
    ...
    ns = null_space(R)
    if ns.shape[1] > 0:
        v = (ns.T)[0] #extract one viable solution
...
def mouse_pressed(event):
    global prev_v, graph_editor_scene, vertices, edges, mode, option_key_down, selected_vertex_idx, nearest_vertex, selected_vertex_idx, mouse_position
