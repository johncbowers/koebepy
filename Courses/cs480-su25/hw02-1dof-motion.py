...
def redraw():
    ...
    if ns.shape[1] == 1 and selected_vertex_idx == -1:
        motion_vertices = vertices
        motion_prev_v   = None
        motion_v        = v
        motion_R        = R
        draw_graph_to_scene(motion_vertices, edges, motion_scene)
        for i in range(1400):
            motion_scene.pushAnimFrame()
            motion_vs = [VectorE2(float(motion_v[i]), float(motion_v[i+1])) for i in range(0, motion_v.shape[0], 2)]
            motion_vertices = [p + motion_vs[i] for i, p in enumerate(motion_vertices)]
            draw_graph_to_scene(motion_vertices, edges, motion_scene)
            motion_R = rigidity_matrix(motion_vertices, edges, pin_edge_idx)
...
def draw_graph_to_scene(vertices, edges, scene):
    scene.addAll([(SegmentE2(vertices[e[0]], vertices[e[1]]), purpleStyle if i == pin_edge_idx else blackStyle) 
                  for i, e in enumerate(edges)])
    scene.addAll([(vertices[i], blackStyle if i != selected_vertex_idx and i != selected_end_vertex_idx else redStyle) 
