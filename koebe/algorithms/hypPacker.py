...
def maximalPacking(diskDcel: DCEL, num_passes: int = 1000, tolerance: float = 3e-10, centerDartIdx: int = -1) -> (DCEL, int):
    ...
    dcel = diskDcel.duplicate(
                vdata_transform = (lambda v : [ExtendedComplex(0,1), 1]), # centered at 0j with radius 1
    ...
    for v in dcel.verts:
        v.data = [ExtendedComplex.ZERO, 0.5]
        v.aim = TWO_PI
    for b in bdryVerts:
        b.data = [ExtendedComplex.ZERO, -5]
        b.aim = -1.0
...
def _set_center(v: Vertex, z: ExtendedComplex) -> None:
    ...
    absz = abs(z)
    if (absz > 1.0):
        v.data[0] = ExtendedComplex(z.z, z.w * absz)#z * (1.0 / absz)
    else:
        v.data[0] = ExtendedComplex(z)
...
def _place_dart_at_origin(aDart: Dart) -> ExtendedComplex:
    ...
    _set_center(a, ExtendedComplex.ZERO)
    
    if s2 <= 0: # If the second has infinite radius
        _set_center(k, ExtendedComplex.ONE)
        ...
        s = x + math.sqrt(x * (x + 2))
        _set_center(k, ExtendedComplex(s / (s + 2)))

# (a - z) / (1 - z * conj(a))
# = (-z + a) / (-z * conj(a) + 1)
# Mobius.mob_trans in CirclePack
def _mob_trans(z: ExtendedComplex, a: ExtendedComplex) -> ExtendedComplex: 
    ...
    Returns: 
        (-z + a) / (-z * conjugate(a) + 1)
    """
    return z.applyMobius(ExtendedComplex(-1), a, -a.conjugate(), ExtendedComplex.ONE)

# Mobius.mobDiscInvValue in CirclePack
def _mobDiskInvValue(w: ExtendedComplex, a: ExtendedComplex, b: ExtendedComplex) -> ExtendedComplex:
...
def _eucl_circle_3(z1: ExtendedComplex, z2: ExtendedComplex, z3: ExtendedComplex) -> (ExtendedComplex, float):
    ...
    return (ExtendedComplex(complex(c_real, c_imag)), c_rad)
    
# HyperbolicMath.h_compcenter in CirclePack
def _compute_center(z1: ExtendedComplex, z2: ExtendedComplex, x1: float, x2: float, x3:float) -> (ExtendedComplex, float):
    ...
    if s1 > 0:
        ...
        if s3 > 0: # third circle is finite
            ...
            if a == b:
                z3 = ExtendedComplex(complex(cc*ahc*ahc, sgn*math.sqrt(1-cc*cc)*ahc))
            else:
                z3 = ExtendedComplex(complex(cc*ahc, sgn*math.sqrt(1-cc*cc)*ahc))
            ...
            sc = (r * r + 1 + 2 * r) / (2 * (1 + r))
            cc2 = cc * cc
            c = ExtendedComplex(complex(sc * cc, sc * sgn * math.sqrt(1-cc2)))
            rad = 1 - sc
            w1 = c - rad
            w2 = c + rad
            w3 = c + ExtendedComplex(rad*1j)
        ...
        t = (d2 - d1) / (d2 * d1 - 1.0)
        newZ = ExtendedComplex.ZERO.applyMobius(
            ExtendedComplex.ONE, 
            ExtendedComplex(-t), 
            ExtendedComplex(-t), 
            ExtendedComplex.ONE)
        theta = ExtendedComplex(complex(0,theta)).apply(1, -t, -t, 1).arg()
        origTheta = (z2 / z1).arg()
        t = (math.sin(theta) / (1.0 - math.cos(theta)) 
             - math.sin(origTheta) / (1.0 - math.cos(origTheta)))
        nextZ = newZ.applyMobius(
...
def _h_horo_center(z1: ExtendedComplex, z2: ExtendedComplex, e2: float, o1: float, o2: float, o3: float):
    ...
    ectr = ExtendedComplex(complex(ectr_real, math.sqrt(a_sq - (ectr_real-0.5)*(ectr_real-0.5))))

    # Find 3 points on the original second circle (this is where e2 is used)
    ecent = (1.0 - e2) * z2
    
    p1 = ecent + ExtendedComplex(complex(0, e2))
    p2 = ExtendedComplex(complex(ecent.imag, ecent.real + e2))
    p3 = ExtendedComplex(complex(ecent.imag, ecent.real - e2))

    One = ExtendedComplex.ONE
    negOne = -One
    Two = ExtendedComplex(complex(2, 0))
    ...
    M2 = Mobius(One, ExtendedComplex(complex(-pp, 0.0)), ExtendedComplex(-pp, 0.0), One)
    ...
    if h > 1e-14:
        p1 = ectr * ((h + r) / h)
        p2 = ectr * ((h - r) / h)
        p3 = ectr + ExtendedComplex(complex(r, 0))
    else:
        p1 = ectr + ExtendedComplex(complex(-r, 0))
        p2 = ectr + ExtendedComplex(complex( r, 0))
        p3 = ectr + ExtendedComplex(complex( 0,-r))
...
def hyperbolic_circle_to_euclidean(h_center: ExtendedComplex, x_rad: float) -> (ExtendedComplex, float):
...
def euclidean_circle_to_hyperbolic(e_center: ExtendedComplex, e_rad: float) -> (ExtendedComplex, float):
    aec = abs(e_center)
    dist = aec + e_rad
