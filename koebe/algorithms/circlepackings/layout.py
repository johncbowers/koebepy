...
def compute_midpoints(packingS2):
    for e in packingS2.edges:
        vi, vj = e.endPoints()
        e.data = vi.data.inversiveMidpointTo(vj.data).inversiveNormalize()
...
def center_of_mass_euclidean(packingS2):
...
def canonical_spherical_layout(packingS2, n_iterations=50):
    ...
    for _ in range(n_iterations):
        ...
        compute_tangencies(packingS2)
        points_to_center = [e.data.toVectorE3().normalize() for e in packingS2.edges]
        center_of_mass = (1/len(points_to_center)) * sum(points_to_center, start=VectorE3(0,0,0))
        
        curr_norm = center_of_mass.norm()
...
def canonical_spherical_layout_v2(packingS2, n_iterations=50):
    """
    ...
    for _ in range(n_iterations):
        packingS2 = packingS2.duplicate(
            vdata_transform=lambda d: d.invertThrough(I1).inversiveNormalize()
        )

        # Compute the current center of mass of the tangency points
        compute_midpoints(packingS2)

        points_to_center = [e.data for e in packingS2.edges]
        center_of_mass = (1/len(points_to_center)) * sum(points_to_center, start=DiskS2(0,0,0,0))
        
        # print(center_of_mass.dualPointOP3.toPointE3().distTo(PointE3.O))

        print(center_of_mass_euclidean(packingS2).norm())

        # Create a DiskS2 object representing the center of mass (note it will have imaginary radius, since this is a timelike ray
        # in the Minkowski spacetime). 
        center_of_mass_disk = center_of_mass
